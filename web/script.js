...
window.onload = async function() {
    ...
    document.getElementById('personSelect').addEventListener('change', handlePersonSwitch);
    document.getElementById('addPersonBtn').addEventListener('click', handleAddPerson);
    document.getElementById('deletePersonBtn').addEventListener('click', handleDeletePerson);
...
async function loadInitialData() {
    ...
    currentPerson = data.current_person;
...
function renderPersonSelect() {
    ...
    // 只剩一人时禁用删除按钮（避免误删）
    document.getElementById('deletePersonBtn').disabled = peopleList.length <= 1;
}

function updatePersonTags() {
    document.querySelectorAll('.person-tag').forEach(el => {
        el.textContent = currentPerson ? `@ ${currentPerson}` : '';
    });
}

async function handlePersonSwitch() {
    const name = document.getElementById('personSelect').value;
    if (!name || name === currentPerson) return;
    await eel.switch_person(name)();
    await loadInitialData();
    await renderCalendar();
    await refreshStats();
}
...
async function handleAddPerson() {
    const name = prompt("请输入新人员姓名：");
    if (!name || !name.trim()) return;
    const ok = await eel.add_person(name.trim())();
    if (!ok) {
        alert(`新增失败：姓名为空或 "${name}" 已存在`);
        return;
    }
    await loadInitialData();
    await renderCalendar();
...
async function handleDeletePerson() {
    ...
    if (!confirm(`确定删除 "${currentPerson}" 的所有记录和设置？此操作不可撤销。`)) return;
    const ok = await eel.delete_person(currentPerson)();
    if (!ok) {
        alert("删除失败。");
        return;
    }
